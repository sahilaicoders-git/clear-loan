  ...
  return (
    <div className="fixed top-6 left-0 right-0 w-full flex justify-center z-[100] px-4 pointer-events-none animate-enter">
      <div className={`w-full flex justify-center transition-all duration-500 ease-[cubic-bezier(0.16,1,0.3,1)] ${isVisible ? 'translate-y-0 opacity-100' : '-translate-y-32 opacity-0'}`}> 
        <nav className="w-full md:w-[700px] lg:w-[900px] bg-white/30 dark:bg-[#09090b]/60 backdrop-blur-2xl border border-white/60 dark:border-white/10 shadow-[0_8px_30px_rgb(0,0,0,0.04)] rounded-full px-6 py-3 pointer-events-auto transition-all duration-500 hover:bg-white/40 dark:hover:bg-[#09090b]/80">
          <div className="flex justify-between items-center w-full">
            ...
            <div className="flex items-center gap-2 sm:gap-4">
              <button
                className="sm:hidden p-2 rounded-full hover:bg-gray-100/50 dark:hover:bg-white/10 text-gray-500 dark:text-gray-400"
              ...
              <div className="hidden sm:flex items-center gap-2 sm:gap-4">
                {user ? (
                  <>
                    <Link to="/loans" className="text-sm font-bold text-gray-600 hover:text-gray-900 dark:text-gray-400 dark:hover:text-white transition-colors">
                      My Portfolio
                    </Link>
                    <div className="h-4 w-px bg-gray-300 dark:bg-gray-700 mx-1 transition-colors"></div>
        ...
        {mobileMenuOpen && (
          <div className="sm:hidden absolute top-20 left-0 right-0 mx-auto w-full max-w-xs bg-white dark:bg-[#09090b] rounded-2xl shadow-lg border border-gray-200 dark:border-gray-800 z-[200] p-6 animate-fade-in flex flex-col gap-4">
            {user ? (
              <>
                <Link to="/loans" className="block text-base font-bold text-gray-700 dark:text-gray-200 py-2" onClick={() => setMobileMenuOpen(false)}>
                  My Portfolio
                </Link>
                <div className="flex flex-col items-start gap-2 mt-2">
                  <span className="text-xs font-bold text-gray-900 dark:text-white leading-none">{user.user_metadata?.full_name || 'User'}</span>
                  ...
                  <button
                    ...
                    className="mt-2 p-2 text-gray-400 hover:text-red-500 dark:hover:text-red-400 transition-colors bg-gray-100/50 dark:bg-white/5 rounded-full"
                    title="Sign Out"
                  >
              ...
              <>
                <Link to="/login" className="block text-base font-bold text-gray-700 dark:text-gray-200 py-2" onClick={() => setMobileMenuOpen(false)}>
                  Log In
                </Link>
                <Link to="/signup" className="block text-base font-bold text-white bg-gray-900 dark:bg-white dark:text-gray-900 rounded-full px-5 py-2 mt-2 text-center shadow-md" onClick={() => setMobileMenuOpen(false)}>
                  Sign Up
                </Link>
              </>
            )}
